...
struct TabBarShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        path.move(to: CGPoint(x: 0, y: isTop ? (isCurveStyle ? height : height-curveSize ) : (isCurveStyle ? 0 : curveSize)))
        ...
        path.addQuadCurve(to: CGPoint(x: width, y: isTop ? (isCurveStyle ? height : height-curveSize ) : (isCurveStyle ? 0 : curveSize)), control: CGPoint(x: width, y: isTop ? height - curveSize : curveSize))
...
#Preview {
    TabBarShape(isTop: false, isCurveStyle: false)
}
